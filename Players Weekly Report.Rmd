...
qb_cpoe <- pbp_rp %>%
  ...
  filter(n_dropbacks >= 30) %>% #Change this No. later in the season
  ungroup() # always ungroup if you no longer need the grouping effect
...
comet_plot <- qb_cpoe %>% 
  ...
  labs(x = "Rate (%)", y = "",
         ...
         subtitle = glue("Difference between **Expected Completion %** and **Actual Completion %**. Minimum 30 passes. Thru Week {current_week}"),
         caption = "Data Source: nflfastR \nGraphic: @steodosescu") +
    theme(plot.title = element_text(face = "bold", 
                                  size = 20, 
                                  hjust = 0.5
...
rushing_plot <- rushing %>% 
  ...
  labs(x = "Success Rate (%)", 
       ...
       subtitle = glue("Minimum 30 Rushes. Thru Week **{current_week}**."),
       caption = "Data Source: nflfastR \nGraphic: @steodosescu"
       ) +
      theme(plot.title = element_text(face = "bold", 
                                  size = 20, 
