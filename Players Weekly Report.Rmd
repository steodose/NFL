...
qbs <- pbp_rp %>%
  ...
  filter(n_dropbacks >= 5) %>% #Change this No. later in the season
...
qb_success_rate <- qbs %>%
  ...
  labs(
    ...
    subtitle = "2022 season, min 5 pass attempts, includes all QB's rush and pass plays. Size represents no. of plays"
  ) +
  theme_custom() +
...
comet_plot <- qb_cpoe %>% 
  ...
  labs(x = "Rate (%)", y = "",
         ...
         subtitle = glue("Difference between **Expected Completion %** and **Actual Completion %**. Minimum 5 passes. Thru Week {current_week}"),
         caption = "Data Source: nflfastR \nGraphic: @steodosescu") +
    theme(plot.title = element_text(face="bold"), text = element_text(family = "Chivo")) +
    theme(plot.subtitle = element_markdown()) +
...
The below shows a summary of QB performances so far...
...
qb_summary_data <- nfl_qb |>
  ...
  arrange(conference, division, -total_passing_tds, -comp_perc, -total_passing_yards)|>
...
qb_summary_table <- qb_summary_data |> 
  select(headshot_url,player_display_name:total_passing_tds, conference:comp_perc) |> 
  gt() |> 
  tab_header(
    title = md("**Quarterback Passing Summary**"), 
    subtitle = glue("Minimum 100 pass attempts. Thru Week {current_week}.")
    )  %>%
  ...
  data_color(
    columns = vars(total_passing_tds),
...
The below shows receiver catch rate againt their average depth of target (air yards).
...
receiving <- pbp_rp %>%
  ...
  filter(targets >= 5) %>% #Change this No. later in the season
  ungroup() # always ungroup if you no longer need the grouping effect
...
receiver_plot <- receiving %>% 
  ...
  labs(x = "aDOT (Yards)", 
       ...
       subtitle = glue("Minimum 5 Targets. Average Depth of Target is based on Air Yards. Thru Week **{current_week}**."),
       caption = "Data Source: nflfastR \nGraphic: @steodosescu"
       ) +
      theme(plot.title = element_text(face="bold"), text = element_text(family = "Chivo")) +
    theme(plot.subtitle = element_markdown())
