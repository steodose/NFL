...
epa_logos <- epa_play %>%
  ...
  labs(
    x = "EPA per Dropback",
    y = "",
    title = "Team QB Efficiency, 2023 NFL Season",
...
qbs <- pbp_rp %>%
  ...
  filter(n_dropbacks >= 80) %>% #Change this No. later in the season
...
qb_success_rate <- qbs %>%
  ...
  labs(
    ...
    subtitle = "2023 season, min 80 pass attempts, includes all QB's rush and pass plays."
  ) +
  theme_custom() +
  theme(plot.title = element_text(face = "bold", 
                                  size = 20, 
...
comet_plot <- qb_cpoe %>% 
  ...
  labs(x = "Rate (%)", y = "",
         ...
         subtitle = glue("Difference between **Expected Completion %** and **Actual Completion %**. Minimum 80 passes. Thru Week {current_week}"),
         caption = "Data Source: nflfastR \nGraphic: @steodosescu") +
    theme(plot.title = element_text(face = "bold", 
                                  size = 20, 
                                  hjust = 0.5
...
receiving <- pbp_rp %>%
  ...
  filter(targets >= 25) %>% #Change this No. later in the season
  ungroup() # always ungroup if you no longer need the grouping effect
...
receiver_plot <- receiving %>% 
  ...
  labs(x = "aDOT (Yards)", 
       ...
       subtitle = glue("Minimum 25 Targets. Average Depth of Target is based on Air Yards. Thru Week **{current_week}**."),
       caption = "Data Source: nflfastR \nGraphic: @steodosescu"
       ) +
      theme(plot.title = element_text(face = "bold", 
                                  size = 20, 
...
rushing_plot <- rushing %>% 
  ...
  #geom_point(size = receiving$targets/20, alpha = 0.5) + 
  # add labels for all players
  geom_text_repel(aes(label = rusher_player_name), color = "grey40", segment.color = "black") + 
  geom_smooth(method = lm, alpha = 0.5, linetype = "dashed", se = FALSE, color = "red", size = 1) + 
  #scale_y_continuous(breaks = c(.4,.45,.5,.55,.6,.65,.7,.75,.8,.85)) +
  theme_custom() + 
  labs(x = "Success Rate (%)", 
       y = "Rush EPA",
       title = "Ground Attack: Performance of NFL Rushers",
