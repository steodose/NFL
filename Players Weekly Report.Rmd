...
data <- load_pbp(2025) #Load PBP data from nflfastR
...
#load data
player_stats <- load_player_stats(seasons = current_season)
...
qb_summary_data <- nfl_qb |>
  ...
  select(headshot_url, player_name, player_display_name, team, completions, attempts, passing_yards, passing_tds, passing_interceptions, passing_epa, sacks_suffered, rushing_yards, rushing_tds)|>
  ...
  summarise(total_completions = sum(completions),
            ...
            total_interceptions = sum(passing_interceptions),
            total_passing_epa = sum(passing_epa),
            total_sacks = sum(sacks_suffered),
            total_rushing_yards = sum(rushing_yards),
            total_rushing_tds = sum(rushing_tds)) |> 
  inner_join(teams_colors_logos|>
               select(team_abbr, team_nick, team_division, team_logo_espn), by=c("team"="team_abbr"))|>
...
qb_summary_table <- qb_summary_data |> 
  select(headshot_url, player_display_name, team_logo_espn, team:total_interceptions, conference:comp_perc, total_passing_epa) |> 
  gt() |> 
  tab_header(
  ...
  tab_options(
    ...
    table.font.names = "Titillium Web", 
...
epa_logos <- epa_play %>%
  ...
  labs(
    x = "EPA per Dropback",
    y = "",
    title = "Team QB Efficiency, 2025 NFL Season",
...
qbs <- pbp_rp %>%
  ...
  filter(n_dropbacks >= 100) %>% #Change this No. later in the season
...
qb_success_rate <- qbs %>%
  ...
  labs(
    ...
    subtitle = "2025 season, min 100 pass attempts, includes all QB's rush and pass plays."
  ) +
  theme_custom() +
  theme(plot.title = element_text(face = "bold", 
                                  size = 20, 
...
#         caption = "Data Source: nflfastR \nGraphic: @steodosescu") +
 #   theme_custom() +
  #  theme(plot.title = element_text(face="bold"), text = element_text(family = "Titillium Web")) +
   # theme(plot.subtitle = element_markdown()) +
    #geom_text(data=qb_cpoe, aes(x=cp_per_db, y=name, label=cp_per_db),
     #         color="#FAAB18", size=2.75, vjust=2.5, family="Titillium Web") +
    #geom_text(data=qb_cpoe, aes(x=exp_cp_per_db, y=name, label=exp_cp_per_db),
     #         color="#1380A1", size=2.75, vjust=2.5, family="Titillium Web") +
    # geom_rect(data=qb_cpoe, aes(xmin=0.82, xmax=0.88, ymin=-Inf, ymax=Inf), fill="lightgrey") +
    #geom_text(data=qb_cpoe, aes(label = cpoe_per_db, y=name, x=0.85), fontface="bold", size=3, # family="Titillium Web")
...
comet_plot <- qb_cpoe %>% 
  ...
  labs(x = "Rate (%)", y = "",
         ...
         subtitle = glue("Difference between **Expected Completion %** and **Actual Completion %**. Minimum 100 passes. Thru Week {current_week}"),
         caption = "Data Source: nflfastR \nGraphic: @steodosescu") +
    theme(plot.title = element_text(face = "bold", 
                                  size = 20, 
                                  hjust = 0.5
    ...
    geom_text(data=qb_cpoe, aes(label = cpoe_per_db, y=name, x=0.85), fontface="bold", size=3, family="Titillium Web") +
  annotate(geom = 'label', x = 0.50, y = 4.5, label = "Less Completions\nThan Expected", family = "Titillium Web", color = "#E64B35FF", fontface = 'bold', fill = "floralwhite", label.size = 0, size = 3) +
  annotate(geom = 'label', x = 0.75, y = 24.5, label = "More Completions\nThan Expected", family = "Titillium Web", color = "#00A087FF", fontface = 'bold', fill = "floralwhite", label.size = 0, size = 3)
...
receiving <- pbp_rp %>%
  ...
  filter(targets >= 80) %>% #Change this No. later in the season
  ungroup() # always ungroup if you no longer need the grouping effect
...
receiver_plot <- receiving %>% 
    ...
    geom_image(aes(image = team_logo_espn), size = 0.030, by = "width", asp = asp_ratio) +
    #geom_point(size = receiving$targets/10, alpha = 0.5) + # change this with targets no.
    ...
    labs(x = "aDOT (Yards)", 
        ...
        subtitle = glue("Minimum 80 Targets. Average Depth of Target is based on Air Yards. Thru Week **{current_week}**."),
        caption = "Data Source: nflfastR \nGraphic: @steodosescu"
        ) +
        theme(plot.title = element_text(face = "bold", 
                                  size = 20, 
...
rushing_plot <- rushing %>% 
  ...
  labs(x = "Success Rate (%)", 
       ...
       subtitle = glue("Minimum 100 Rushes. Thru Week **{current_week}**."),
       caption = "Data Source: nflfastR \nGraphic: @steodosescu"
       ) +
      theme(plot.title = element_text(face = "bold", 
                                  size = 20, 
