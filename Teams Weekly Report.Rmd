...
epa_plot <- off_def_epa %>% 
    ggplot(aes(x = off_epa, y = def_epa)) + #Change image to "team_wordmark" when SSL certificate issue is resolved
    geom_image(aes(image = team_wordmark), size = 0.095, by = "width", asp = asp_ratio) +  #Change size back to 0.065 when SSL certificate issue is resolved
...
epa_logos <- epa_play %>%
  ...
  geom_image(aes(image = team_wordmark), size = 0.065, by = "width", asp = asp_ratio) + #Change image size to 0.045 when SSL certificate issue is resolved.
...
epa_cpoe_logos <- epa_play %>%
  ...
  geom_image(aes(image = team_wordmark), size = 0.065, by = "width", asp = asp_ratio) +
  scale_fill_identity(aesthetics = c("fill", "colour")) +
  theme_custom() +
  geom_hline(yintercept = 0) +
  labs(
...
explosive_plays_plot <- explosive_plays %>% 
  ...
  annotate("text", x = 0.080, y = "PIT", label = "2020 NFL Avg = 6.8%", color = "red") +
...
average_go_for_it_rate <- mean(go_for_it$`2010-2021`) #Get mean go for it rates to put inline with R code down below
...
fourth_down_table <- go_for_it_by_year %>%
  ...
  tab_header(title = md("**4th Down Decison Making**"),
             subtitle = glue("Percentages shown are how often a team went for it when it on 4th & short, defined as less than 3 yards to go, and when win probability was between 20% and 80% (game-neutral situations). 2021 data is thru {current_week}.")) %>%
  tab_source_note(
