...
qb_success_rate <- qbs %>%
  ...
  labs(
    ...
    subtitle = "2020 season, min 50 pass attempts, includes all QB's rush and pass plays. Size represents no. of plays"
  ) +
  theme_custom() +
...
receiver_plot <- receiving %>% 
  ...
  labs(x = "aDOT (Yards)", 
       ...
       subtitle = glue("Minimum 50 Targets. Average Depth of Target is based on Air Yards. Thru Week **{current_week}**."),
       caption = "Data Source: nflfastR \nGraphic: @steodosescu"
       ) +
      theme(plot.title = element_text(face="bold"), text = element_text(family = "Chivo")) +
    theme(plot.subtitle = element_markdown())
