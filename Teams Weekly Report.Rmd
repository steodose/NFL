...
epa_plot <- off_def_epa %>% 
    ...
    geom_image(aes(image = team_logo_espn), size = 0.065, by = "width", asp = asp_ratio) +  #Change size back to 0.065 when SSL certificate issue is resolved
...
epa_logos <- epa_play %>%
  ...
  geom_image(aes(image = team_logo_espn), size = 0.045, by = "width", asp = asp_ratio) + #Change image size to 0.045 when SSL certificate issue is resolved.
