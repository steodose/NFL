...
epa_plot <- off_def_epa %>% 
  ...
  geom_abline(slope = -1.5, intercept = c(.4, .3, .2, .1, 0, -.1, -.2, -.3, -.4), alpha = .2) +
    labs(x = "Offense EPA/play",
        ...
        subtitle = glue("Rush and pass plays only. Thru **Week Regular Season**.")) +
...
point_diff_barplot <- games_df %>% 
  ...
  labs(x = "", 
       ...
       subtitle = glue("Thru Regular Season")) +
  theme(plot.title.position = "plot",
        plot.title = element_text(face = "bold", 
                                  size = 20, 
                                  hjust = 0.5
...
explosive_plays_plot_logos <- explosive_plays %>% 
  ...
  labs(
    ...
    subtitle = glue("Explosive plays = 15 yards+ on rushes, 20 yards+ on passes. Thru Regular Season.")
  ) +
  theme_custom() +
...
fourth_down_table <- go_for_it_by_year %>%
  ...
  tab_header(title = md("**4th Down Decison Making**"),
             subtitle = glue("Percentages shown are how often a team went for it when it on 4th & short, defined as less than 3 yards to go, and when win probability was between 20% and 80% (game-neutral situations). 2022 data is thru Regular Season.")) %>%
  tab_source_note(
