...
# transformations
games_epa <- games_epa %>%
  select(game_id, week, team_home = home_team, team_away = away_team, away_epa_play, home_epa_play) %>% 
  pivot_longer(contains('team'), names_to = 'home_away', values_to = 'team', names_prefix = 'team_') %>%
  mutate( #this ensures you grab the opposing team's epa_play for SOS analysis
    opp_epa_play = ifelse(home_away == 'home', away_epa_play, home_epa_play)
  ) %>% 
  group_by(team) %>%
  summarise(
...
fourth_down_table <- go_for_it_by_year %>%
  ...
  tab_header(title = md("**4th Down Decison Making**"),
             subtitle = glue("Percentages shown are how often a team went for it when it on 4th & short, defined as less than 3 yards to go, and when win probability was between 20% and 80% (game-neutral situations). 2023 data is Thru Week {current_week}.")) %>%
  tab_source_note(
