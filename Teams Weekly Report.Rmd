...
explosive_plays_plot_logos <- explosive_plays %>% 
  ...
  labs(
    ...
    y = "",
    caption = "Data: @nflscrapR | Plot: @steodosescu",
    title = glue("Most Explosive Teams, 2023 NFL Season"),
...
go_for_it <- fourth_down_plays %>%
  ...
  summarize(n = n()) %>% 
  mutate(`2013-2023` = round(100 * (n / sum(n)), 1)) %>%
  select(-c(n)) %>% 
  pivot_wider(names_from = "season", values_from = "2013-2023") %>%
...
average_go_for_it_rate <- mean(go_for_it$`2013-2023
                               `) #Get mean go for it rates to put inline with R code down below

# Look at  trends
trend <- go_for_it %>%
  ungroup() %>%
  select(team = posteam, `2013-2023`) %>%
  group_by(team) %>%
  mutate(`2013-2023` = list(`2013-2023`)) %>%
  distinct(team, `2013-2023`) %>%
  ungroup()

# Go for it data by year
go_for_it_by_year <- go_for_it %>%
  select(season, team = posteam, `2013-2023`) %>%
  pivot_wider(names_from = "season", values_from = "2013-2023") %>%
  mutate_if(is.numeric, list(~replace_na(., 0))) %>% 
  ungroup() %>%
...
fourth_down_table <- go_for_it_by_year %>%
  select(team_logo_espn, team, `2013`, `2014`, `2015`, `2016`, `2017`, `2018`, `2019`, `2020`, `2021`, `2022`, `2023`) %>%
  arrange(desc(`2023`)) %>% 
  rowwise() %>% 
  mutate(`2013-2023` = list(c(`2013`, `2014`, `2015`, `2016`, `2017`, `2018`, `2019`, `2020`, `2021`, `2022`, `2023`))) %>% 
  gt() %>%
  data_color(
    columns = 3:13, 
  ...
  tab_header(title = md("**4th Down Decison Making**"),
             subtitle = glue("Percentages shown are how often a team went for it when it on 4th & short, defined as less than 3 yards to go, and when win probability was between 20% and 80% (game-neutral situations). 2023 data is thru Regular Season.")) %>%
  tab_source_note(
    source_note = md("DATA: nflfastR<br>TABLE: @steodosescu")) %>% 
  gt_plt_sparkline(`2013-2023`)
