...
py_wins_plot <- pyth_wins_df %>% 
  ...
  labs(x = "Actual Win %",
       ...
       subtitle = glue("Pythagorean Win % tells how many games a team should have won based on points scored/allowed. It's an indicator of potential future success. Thru Week {current_week}.")) +
  theme(plot.title.position = "plot",
        plot.title = element_text(face = "bold", 
                                  size = 20, 
                                  hjust = 0.5
...
# nflplotR::element_nfl_logo() turns team abbreviations on the axis into logos.
# This replaces an earlier approach that embedded <img> tags and rendered them
# with ggtext::element_markdown(), which stopped working under ggplot2 4.0.
...
explosive_plays_plot_logos <- explosive_plays %>%
  ...
  labs(
    ...
    subtitle = glue("Explosive plays = 10 yards+ on rushes, 20 yards+ on passes. Thru Week {current_week}.")
  ) +
  theme_custom() +
  ...
  theme(axis.text.y = nflplotR::element_nfl_logo(size = 1)) +
