...
# Create Bengals-Chiefs plot

cin_kc_wp_plot <- game_data %>%
  filter(!is.na(home_wp),
         !is.na(away_wp)) %>%
  dplyr::select(game_seconds_remaining,
                home_wp,
  ...
  #geom_image(x = -200, y = 0.18, image = kc_logo, size = 0.08, asp = 1.5) +
  geom_hline(yintercept = 0.5, color = "gray", linetype = "dashed") +
  scale_color_manual(labels = c("CIN", "KC"),
                     values = c(cin_color, kc_color),
                     guide = FALSE) +
  #scale_x_reverse(breaks = seq(0, 3600, 300)) + 
  geom_vline(xintercept = 900, linetype = "dashed") + 
  geom_vline(xintercept = 1800, linetype = "dashed") + 
  ...
  labs(
    ...
    subtitle = glue("<span style = 'color:{cin_color}';'>**Cincinnati Bengals**</span> vs. <span style = 'color:{kc_color}';'>**Kansas City Chiefs**</span>. GEI: {wp_gei}")
  ) + 
  theme_custom() +
  theme(plot.title = element_text(face="bold")) +
  scale_x_continuous(
    trans = "reverse",
    breaks = c(2700, 1800, 900, 0, -(max_sec-min_sec)), 
    limits = c(3700, -(max_sec-min_sec)),
    labels = c("END\nQ1", "HALF\nTIME", "END\nQ3", "END\nQ4", "END\nOT")) +
  scale_y_continuous(labels = scales::percent_format()) +
  theme(plot.subtitle = element_markdown())
...
# Create 49ers vs Rams plot
sf_la_wp_plot <- game_data %>%
  filter(!is.na(home_wp),
         !is.na(away_wp)) %>%
  dplyr::select(game_seconds_remaining,
                home_wp,
  ...
  geom_line(size = 1.2) +
  geom_hline(yintercept = 0.5, color = "gray", linetype = "dashed") +
  scale_color_manual(labels = c("SF", "LA"),
                     values = c(sf_color, gb_color),
                     guide = FALSE) +
  #scale_x_reverse(breaks = seq(0, 3600, 300)) + 
  geom_vline(xintercept = 900, linetype = "dashed") + 
  ...
  labs(
    ...
    y = "",
    title = "",
    subtitle = glue("<span style = 'color:{sf_color}';'>**San Francisco 49ers**</span> vs. <span style = 'color:{la_color}';'>**Los Angeles Rams**</span>. GEI: {wp_gei}"),
    caption = "Data and WP model from nflfastR | Plot: @steodosescu"
  ) + 
  theme_custom() +
  theme(plot.title = element_text(face="bold")) +
...
cin_kc_wp_plot / sf_la_wp_plot
ggsave("Conf Champ Win Probability Patchwork Plot.png")


# Add logo to plot
plot_with_logo <- add_logo(
  plot_path = "/Users/Stephan/Desktop/R Projects/NFL/Conf Champ Win Probability Patchwork Plot.png", # url or local file for the plot
  logo_path = "/Users/Stephan/Desktop/R Projects/NFL/nfl-logo.png", # url or local file for the logo
  logo_position = "bottom left", # choose a corner
  # 'top left', 'top right', 'bottom left' or 'bottom right'
  logo_scale = 30
...
# save the image and write to working directory
magick::image_write(plot_with_logo, "Conf Champ Win Probability Patchwork Plot with Logo.png")


## Super Bowl
game_data <- pbp_data %>%
  filter(game_id == "2021_22_LA_CIN") 
...
# Create Bengals vs Rams Super Bowl plot
la_cin_wp_plot <- game_data %>%
  filter(!is.na(home_wp),
         !is.na(away_wp)) %>%
  dplyr::select(game_seconds_remaining,
                home_wp,
  ...
  labs(
    ...
    y = "",
    title = "",
    subtitle = glue("<span style = 'color:{la_color}';'>**Los Angeles Rams**</span> vs. <span style = 'color:{cin_color}';'>**Cincinnati Bengals**</span>. GEI: {wp_gei}"),
    caption = "Data and WP model from nflfastR | Plot: @steodosescu"
  ) + 
  theme_custom() +
  theme(plot.title = element_text(face="bold")) +
...
playoff_games <- playoff_games %>% 
  slice(-(2:3)) %>% 
  slice(-(14:16)) #slice off the duplicates that were introduced as part of the joins (bc there's more than 1 Josh Allen). 
