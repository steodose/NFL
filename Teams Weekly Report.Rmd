...
expected_totals_barplot <- joined_df %>% 
  ...
  labs(x = "", 
       ...
       subtitle = glue("Actual win totals vs. Predicted. Estimated totals from Bet365 sportsbook as of August 26, 2022.")) +
...
epa_logos <- epa_play %>%
  ...
  labs(
    x = "EPA per Dropback",
    y = "",
    title = "Quarterback Efficiency, 2022 NFL Season",
...
epa_cpoe_logos <- epa_play %>%
  ...
  labs(
    ...
    title = "Quarterback Performance, 2022 NFL Season",
...
explosive_plays_plot_logos <- explosive_plays %>% 
  ...
  labs(
    ...
    y = "",
    caption = "Data: @nflscrapR | Plot: @steodosescu",
    title = glue("Most Explosive Teams, 2022 NFL Season"),
...
go_for_it <- fourth_down_plays %>%
  ...
  summarize(n = n()) %>% 
  mutate(`2013-2022` = round(100 * (n / sum(n)), 1)) %>%
  select(-c(n)) %>% 
  pivot_wider(names_from = "season", values_from = "2013-2022") %>%
...
average_go_for_it_rate <- mean(go_for_it$`2013-2022`) #Get mean go for it rates to put inline with R code down below

# Look at  trends
trend <- go_for_it %>%
  ungroup() %>%
  select(team = posteam, `2013-2022`) %>%
  group_by(team) %>%
  mutate(`2013-2022` = list(`2013-2022`)) %>%
  distinct(team, `2013-2022`) %>%
  ungroup()

# Go for it data by year
go_for_it_by_year <- go_for_it %>%
  select(season, team = posteam, `2013-2022`) %>%
  pivot_wider(names_from = "season", values_from = "2013-2022") %>%
  mutate_if(is.numeric, list(~replace_na(., 0))) %>% 
  ungroup() %>%
...
fourth_down_table <- go_for_it_by_year %>%
  select(team_logo_espn, team, `2013`, `2014`, `2015`, `2016`, `2017`, `2018`, `2019`, `2020`, `2021`, `2022`) %>%
  arrange(desc(`2022`)) %>% 
  rowwise() %>% 
  mutate(`2013-2022` = list(c(`2013`, `2014`, `2015`, `2016`, `2017`, `2018`, `2019`, `2020`, `2021`, `2022`))) %>% 
  gt() %>%
  data_color(
    columns = 3:12, 
  ...
  tab_header(title = md("**4th Down Decison Making**"),
             subtitle = glue("Percentages shown are how often a team went for it when it on 4th & short, defined as less than 3 yards to go, and when win probability was between 20% and 80% (game-neutral situations). 2022 data is thru Week {current_week}.")) %>%
  tab_source_note(
    source_note = md("DATA: nflfastR<br>TABLE: @steodosescu")) %>% 
  gt_plt_sparkline(`2013-2022`)
