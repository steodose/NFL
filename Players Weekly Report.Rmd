...
receiving <- pbp_rp %>%
  ...
  filter(targets >= 20) %>% #Change this No. later in the season
  ungroup() # always ungroup if you no longer need the grouping effect
...
receiver_plot <- receiving %>% 
  ...
  labs(x = "aDOT (Yards)", 
       ...
       subtitle = glue("Minimum 20 Targets. Average Depth of Target is based on Air Yards. Thru Week **{current_week}**."),
       caption = "Data Source: nflfastR \nGraphic: @steodosescu"
       ) +
      theme(plot.title = element_text(face = "bold", 
                                  size = 20, 
